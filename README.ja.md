...
agbd は Git の内部コマンドを利用して対象ブランチの一覧を取得し、最終コミット日時・メッセージ・マージ済み状態などを表示します。保護ブランチは名前の完全一致のほか、プレフィックス一致（`release` を指定すると `release/` 配下もすべて保護）や `/regex/` 形式の正規表現でも指定可能です。
